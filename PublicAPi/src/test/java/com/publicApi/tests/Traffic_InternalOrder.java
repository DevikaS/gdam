 ...
 */
public class Traffic_InternalOrder extends OrdersBaseTest{

    @Test
    public void trafficInternalOrderTest() {
        ...
        waitFor(60000); // Wait for order to completely save to Library so that 'qcAssetId' is generated

        String assetId=apiCall.getQcAssetIdForConfirmedOrders(getOrderId());

        responsePayLoad = apiCall.createInternalOrder();
        setOrderId(responsePayLoad.getId());
        ...
        responseStr=apiCall.addDestinationItems(getOrderId(), getItemId());
        setDestinationId(ResponseParser.getDestinationId(responseStr));
       ...
       // Verification: Check that Internal orders are not visible/available to BU users
        String actual =apiCall.listOrdersWithJsonResponse();

        Assert.assertTrue(!actual.contains(getOrderId()),
                this.getClass().getSimpleName().toUpperCase()+": " +
                        "End Point Failed as Internal Orders are visible to Users, Internal_Order_ID: "+getOrderId());
    }
}